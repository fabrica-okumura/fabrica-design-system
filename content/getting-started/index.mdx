...
```bash
npm install @fabrica_communications/design-system
```

### 2. CSS読み込み

`src/app/globals.css` などで CSS を読み込みます。

```css
@import "@fabrica_communications/design-system/styles.css";
```
...
- `peerDependencies` の `react` / `react-dom` / `next` はスターター側で合わせる必要があります。
...
- `src/components/shared` は横断的に使う中レベル UI
- `asChild` でタグ差し替えを許可
